...
use ethers::types::{
    transaction::eip2718::TypedTransaction, Address, Bytes, NameOrAddress, TransactionRequest, U256,
};
...
impl TxSubmitter {
    ...
    pub async fn submit_batch(&self, sweeps: &[SweepJob]) -> Result<String> {
        ...
        self.update_sweep_status(&ids, SweepStatus::Submitted)
            .await?;
        ...
        match confirmed {
            Ok(block) => {
                ...
                self.update_sweep_status(&ids, SweepStatus::Confirmed)
                    .await?;
                for sweep in sweeps {
                    sqlx::query(
    ...
    fn encode_batch(&self, sweeps: &[SweepJob]) -> Vec<u8> {
        ...
        let selector =
            &ethers::utils::keccak256(b"batchSweep((address,address,address,uint256)[],uint256)")
                [..4];
    ...
    async fn wait_for_confirmation(&self, tx_hash: ethers::types::H256) -> Result<u64> {
    ...
    async fn update_sweep_status(&self, ids: &[Uuid], status: SweepStatus) -> Result<()> {
        for id in ids {
            sqlx::query("UPDATE sweep_jobs SET status = $1, updated_at = NOW() WHERE id = $2")
                .bind(&status)
                .bind(id)
                .execute(&self.pool)
                .await?;
